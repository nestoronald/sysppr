...
xajax={};if('undefined'==typeof xajax.config){xajax.config={};}
xajax.default_config={commonHeaders:{'If-Modified-Since':'Sat, 1 Jan 2000 00:00:00 GMT'},postHeaders:{},getHeaders:{},waitCursor:false,statusMessages:false,'baseDocument':document,'requestURI':document.URL,defaultMode:'asynchronous',defaultHttpVersion:'HTTP/1.1',defaultContentType:'application/x-www-form-urlencoded',defaultResponseDelayTime:1000,defaultExpirationTime:10000,defaultMethod:'POST',defaultRetry:5,defaultReturnValue:false,maxObjectDepth:20,maxObjectSize:2000,responseQueueSize:1000};xajax.config.setDefaults=function(){for(a in xajax.default_config){if('undefined'==typeof xajax.config[a]){xajax.config[a]=xajax.default_config[a];}}}
...
xajax.tools.in_array=function(array,valueToCheck){for(i in array)
...
return'';var elWorkspace=xajax.$('xajax_temp_workspace');if(!elWorkspace)
{elWorkspace=oDoc.createElement('div');elWorkspace.setAttribute('id','xajax_temp_workspace');elWorkspace.style.display='none';elWorkspace.style.visibility='hidden';oDoc.body.appendChild(elWorkspace);}
...
{var iLen=children.length;for(var i=0;i<iLen;++i){var child=children[i];if(('undefined'!=typeof child.childNodes)&&(child.type!='select-one')&&(child.type!='select-multiple'))
...
xajax.tools.json.processFragment=function(nodes,seq,oRet,oRequest){var xx=xajax;var xt=xx.tools;for(nodeName in nodes){if('xjxobj'==nodeName){for(a in nodes[nodeName])
{var obj=nodes[nodeName][a];obj.fullName='*unknown*';obj.sequence=seq;obj.request=oRequest;obj.context=oRequest.context;xt.queue.push(xx.response,obj);++seq;}}else if('xjxrv'==nodeName){oRet=nodes[nodeName];}else if('debugmsg'==nodeName){txt=nodes[nodeName];}else
...
return oRet;}
xajax.js={}
xajax.js.includeScriptOnce=function(command){command.fullName='includeScriptOnce';var fileName=command.data;var oDoc=xajax.config.baseDocument;var loadedScripts=oDoc.getElementsByTagName('script');for(i in loadedScripts)
{var script=loadedScripts[i];if(script.src)
...
xjx.$=xajax.tools.$;xjx.getFormValues=xajax.tools.getFormValues;xjx.request=xajax.request;if('undefined'==typeof JSON)xajax.js.includeScript({data:xajax.config.JavaScriptURI+'xajax_js/JSON.js'});xajax.config.setDefaults();