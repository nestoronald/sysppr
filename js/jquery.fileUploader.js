...
(function($) {
	...
	$.fn.fileUploader = function(config){
		...
		config = $.extend({}, {
			...
			afterUpload: function(){xajax_reloadFileTree(document.getElementById("hiddenIdPpr").value);},
			afterEachUpload: function(){xajax_reloadFileTree(document.getElementById("hiddenIdPpr").value)} //callback after each file has been uploaded
		...
		*/
		function beforeEachUpload($form) {
			...
			$uploadData = $form.parent();
			$uploadData.find('.status').text('Subiendo...');
