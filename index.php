...
/*---*/
	...
	function actividadproducto($idsubcategory,$idcategory){
		...
		//$result=selectPPR($idsubcategory);
		// $html="<h3>Año: <span id='name_year'></span></h3>";
		// $html.="<h3>Programa: <span id='name_prog'></span> </h3>";
		$html =' ';
		...
		$objResponse->assign("result_details","innerHTML","");
		...
		$objResponse->script('
					...
					// id = $(".list_product" ).closest("ul").closest("li").find("span").html();
					// alert(id)
					$( ".list_product" ).click(function(){

					    // var year= $(this).parents("li.year_list").find("span").html();
					    // var prog= $(this).parents("li.prog_list").find("span").html();
					    // html = year+" >> "+prog;
					    // $("body").find("#lbl-prog").html(prog);
					    // $("body").find("#lbl-year").html(year);
					    // $("#navigp").html(html);
					    // alert(html);
	...
	{
		$objResponse=new xajaxResponse();
		$objResponse->script('$( ".list_product" ).click(function(){
					    var id = $(this).closest("ul").closest("li").find("span").html();
					    //alert(id);
					    $("body").find("#lbl-prog").html(id);
					});');
		return $objResponse;
	}
//nrl
	function subcategory_detalle($idsubcategory,$idcategory){
		...
		if (isset($res_01["year_description"])) {
			$year_title =$res_01["year_description"][0];
			$smarty->assign("title_y",$year_title);
	...
	function secureLogin($form){
		...
		if(isset($form['user'], $form['p'])) {
			...
			if (!$result["error"]){
				...
				$objResponse->assign("navigp","innerHTML","");
				$objResponse->assign("divactividadproducto","innerHTML","");
				$objResponse->assign("result_details","innerHTML","");
				$objResponse->assign("formLogin","innerHTML","");
				$objResponse->assign("projectContent","innerHTML","");
				$objResponse->assign("login_home","innerHTML","");
				$objResponse->assign("indexContent","innerHTML","");
				$objResponse->assign("loginContent","innerHTML","");
				$objResponse->assign("menuppr","innerHTML","");
	...
	function NewCategory(){
		...
		$html="<form id=\"nCategory\"><div id=\"newcategory\" class='new'>
		    ...
		    <textarea rows='3'  name='c_description'></textarea>
		    <span id='msj_cdescription' class='msjalert'></span>

          	<label for=\"c_idyear\"> Selecione año</label><div> ";
	...
	function ppr_menu($act="")
	{
		$objResponse = new xajaxResponse();
		$label = ($act=="ppr")?"Productos y Actividades":"Resultados";
		$idlbl = ($act=="ppr")?"lbl-prod-act":"lbl-resul";
		$html='<li class="firts" id="'.$idlbl.'" ><a href="#" >'.$label.' <b class="caret"></b></a>
				<ul class="second '.$act.' navbar-inner">';
		$result= selectcategory_year($idyear="enable",$action="y");
		//anio
		for ($i=0; $i < $result["Count"]; $i++) {
			$html.='<li class="year_list">
					<a href="#"><span class="yearlbl">'.$result["year_description"][$i].' </span><b class="caret"></b></a>
					<div class="dropdown_column navbar-inner">
					';
					$resultCategory=CategoryYearQuery($result["idyear"][$i]);
					//Programa
					 if ($resultCategory["Error"]==0) {
						$html .= "<!--span class='leyenda'> <span>Programas:</span></span-->
								<ul>";
						for($j=0; $j<$resultCategory["Count"]; $j++){
							$html.="<li class='prog_list' value=".$resultCategory["id_years_category"][$j]." ><span>".$resultCategory["category_description"][$j]."</span>";
							// $html.=menu_item_producto($resultCategory["id_years_category"][$j],$act);
							$html.=menu_item_producto($resultCategory["id_years_category"][$j],$act);
							$html.="</li>";
							}
						$html .= "</ul>";
	...
	function menu_item_producto($id_years_category,$act){
		$html = "";
		$resultSubCategory=YearCat_SubcatQuery($id_years_category);
		if ($resultSubCategory["Error"]==0) {
			$html = "<!--span class='leyenda'><span>Productos:</span></span-->
					<ul>";
			for ($i=0; $i <count($resultSubCategory["subcategory_description"]); $i++) {
				$html .="<li class='prod_list' value='".$resultSubCategory["idsubcategory"][$i]."'>";
				if ($act=="ppr") {
					$html .="<a class='list_product' href='#' onclick='xajax_actividadproducto(".$resultSubCategory["idsubcategory"][$i].",".$id_years_category.");'>".$resultSubCategory["subcategory_description"][$i]."</a>";
				}
				else {
					$html .="<span>".$resultSubCategory["subcategory_description"][$i]."</span>";
					// $resultFilterActivity=product_details($resultSubCategory["idsubcategory"][$i], $id_years_category, $idactivity="");
					$resultFilterActivity=YCSubcat_ActivityQuery($resultSubCategory["id_cat_subcat"][$i]);
					$html .= "<ul>";
					for ($j=0; $j < $resultFilterActivity["Count"]; $j++) {
						$html.="<li>";
						// $html.="<a class='list_act' href='#' onclick='xajax_resultdetails(".$resultSubCategory["idsubcategory"][$i].",".$id_years_category.",".$resultFilterActivity["idactivity"][$j].",999)'>".$resultFilterActivity["activity_description"][$j]."</a>";
						$html.="<a class='list_act' href='#' onclick='xajax_resultdetails(".$resultFilterActivity["id_uf"][$j].")'>".$resultFilterActivity["activity_description"][$j]."</a>";
						$html.="</li>";
					}
					$html .= "</ul>";
				}
	...
	function resultdetails($id_uf=0){
		$objResponse = new xajaxResponse();
			// $result_1 = result_upload($idsubcategory, $idcategory, $idactivity);
			$result_1 = UF_UploadQuery($id_uf);
			$html = '';
			 $dir= "results/result_";
			 //$objResponse->alert(print_r($result_1,TRUE));

			if ( $result_1["Count"] > 0) {
				$html .= '<h4>Actividad: <span id="act_name_r">'.$result_1["activity_description"][0].'</span></h4>';
					 ...
					 if ($result_1["idupload"][$i]!=7) {
					 	if (isset($arrayResult["file_main"])) {
					 		if (file_exists($dir.$result_1["idupload"][$i].'/'.trim($arrayResult["file_main"]).'.pdf')) {
					 			$html .= '<span class="plus-download f-right"><small>Informe Tecnico: </small><a target="_blank"  href="'.$dir.$result_1["idupload"][$i].'/'.$arrayResult["file_main"].'.pdf" rel="nofollow" class="view_details">[+Leer Mas] </a>&nbsp &nbsp
			   ...
			   if (!empty($arrayResult["anexos"]["file"]) ) {
				...
				if (!empty($arrayResult["mapas"]) ) {
                                        $html .=' <h4 class="r-title">Mapas: </h4>
                                            <ul class="cat-mapas img-mapas">';
                                        for ($j=0; $j<count($arrayResult["mapas"]["file"]); $j++){
                                                $html .= '<li> <a title="demo'.$j.'" class="map-gallery map-gallery'.$i.'" rel="group'.$i.'" href="'.$dir.$result_1["idupload"][$i].'/mapas/medium/'.$arrayResult["mapas"]["file"][($j)].'">
                                                <img src="'.$dir.$result_1["idupload"][$i].'/mapas/small/'.$arrayResult["mapas"]["file"][$j].'"></a></li>';
                                        }
                                }
	...
	$xajax->registerFunction('load_script');
	...
	$smarty->assign("ppr_menu",ppr_menu("ppr"));
	$smarty->assign("result_menu",ppr_menu("result"));
